...
function createDrawingArea() {
    ...
    game.create.grid('drawingGrid', 28 * canvasZoom, 28 * canvasZoom, canvasZoom, canvasZoom, 'rgba(0,191,243,0.8)');
...
function paint(pointer) {
    ...
    else
    {
        for (var q = -1; q < 2; q++) {
            for (var w = -1; w < 2; w++) {
                 data[y+q][x+w] = 1;
                canvas.rect((x+w) * canvasZoom, (y+q) * canvasZoom, canvasZoom, canvasZoom, color);
                preview.rect((x+w) * previewSize, (y+q) * previewSize, previewSize, previewSize, color);
            }
        }
    }
